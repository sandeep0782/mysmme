...
type AdminOrder = {
  ...
  /*
   * Seller-order style response:
   * order contains parent/customer order.
   */
  order?: ParentOrder;

  /*
   * Direct admin/master-order response fallback.
   */
  user?: Customer;
  customer?: Customer;
...
const AdminOrderDetailsPage = () => {
  ...
  const order: AdminOrder | undefined =
    orderResponse?.data ?? orderResponse?.order;
  ...
  const customer = order?.order?.user ?? order?.user ?? order?.customer;

  const shippingAddress =
    order?.order?.shippingAddress ?? order?.shippingAddress;
      ...
      <div className="flex min-h-screen flex-col items-center justify-center bg-slate-50 px-5">
        <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-slate-500">
            The order could not be found or something went wrong while fetching
            it.
          </p>
          ...
          <div className="mt-5 flex justify-center gap-3">
            ...
            <button
              type="button"
              onClick={() => router.push("/platform/admin/orders")}
          ...
          {error && (
            <p className="mt-4 text-xs text-red-500">
              Please check the API response or server logs.
            </p>
          )}
        </div>
      </div>
  ...
  const rawStatus = order.status ?? order.orderStatus ?? order.order?.status;
  ...
  const displayOrderNumber =
    order.orderNumber ?? order.orderId ?? order.order?.orderNumber ?? order._id;
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-slate-500">
                <div className="flex items-center gap-1.5">
                  ...
                  <span>
                    Order placed on{" "}
                    {formatDate(order.createdAt ?? order.order?.createdAt)}
                  </span>
                </div>
              </div>
        ...
        <div className="grid gap-6 xl:grid-cols-3">
          ...
          <div className="space-y-6 xl:col-span-2">
            ...
            <section className="rounded-2xl border border-slate-200 bg-gradient-to-br from-white to-slate-50 p-6 shadow-sm">
              ...
              <div className="max-h-[460px] divide-y divide-slate-100 overflow-y-auto pr-2">
                {order.items?.length ? (
                  order.items.map((item, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex h-20 w-20 flex-shrink-0 items-center justify-center overflow-hidden rounded-xl border border-slate-200 bg-slate-100">
                          {productImage ? (
                            <img
                              src={productImage}
                              alt={productName}
              ...
              <div className="rounded-xl border border-blue-100 bg-white/80 p-4">
                ...
                {shippingAddress ? (
                  <p className="mt-2 text-sm leading-6 text-slate-600">
                    {shippingAddress.addressLine1 || "Address not available"}
                ...
                <section className="rounded-2xl border border-slate-200 bg-gradient-to-br from-white to-indigo-50/40 p-6 shadow-sm">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    {order.shipping.courierName && (
                      <DetailBox
                        label="Courier"
                        value={order.shipping.courierName}
                      />
                    )}
                    ...
                    {order.shipping.dispatchedAt && (
                      <DetailBox
                        label="Dispatched At"
                        value={formatDateTime(order.shipping.dispatchedAt)}
                      />
                    )}
                    ...
                    {order.shipping.deliveredAt && (
                      <DetailBox
                        label="Delivered At"
                        value={formatDateTime(order.shipping.deliveredAt)}
                      />
                    )}
                  </div>
          ...
          <div className="space-y-6">
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="space-y-4">
                <InfoRow label="Order Number" value={displayOrderNumber} />
                ...
                {order.seller && (
                  <InfoRow label="Seller ID" value={order.seller} />
                )}
                ...
                <InfoRow
                  ...
                  value={formatText(paymentStatus)}
                />
                ...
                {(order.updatedAt || order.order?.updatedAt) && (
                  <InfoRow
                    label="Last Updated"
                    value={formatDateTime(
                      order.updatedAt ?? order.order?.updatedAt,
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs leading-5 text-slate-500">
                      Make sure the products are available and ready before
                      moving this order to the next stage.
                    </p>
                  </div>
                </div>
              </section>
...
const PaymentStatusBadge = ({ status }: { status: string }) => {
  ...
  if (
    ...
    className = "border-emerald-200 bg-emerald-50 text-emerald-700";
  } else if (normalized === "pending" || normalized === "created") {
    className = "border-amber-200 bg-amber-50 text-amber-700";
